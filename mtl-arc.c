...
typedef enum { type_num, type_sym, type_cons, type_fn, type_builtin } atype;
...
atom *findsym(const char *name) {
  for(atom *symlist = symbol_root; !no(symlist); symlist = cdr(symlist))
    if(!strcmp(name, symval(car(symlist))))
      return symlist;
  return nil;
}
...
atom *env_get(atom *env, atom *sym) {
	atom *parent = car(env), *bs = cdr(env), *b = nil;
	while (!no(bs)) {
		b = car(bs);
		if (car(b) == sym)
			return cdr(b);
...
char *gettoken() {
  ...
  do { if((ch = getc(ifp)) == EOF) exit(0); } while(isspace(ch));
  add_to_buf(ch);
...
atom *readlist() {
  ...
  putback_token(token);
  tmp = readexpr(); /* Must force evaluation order */
